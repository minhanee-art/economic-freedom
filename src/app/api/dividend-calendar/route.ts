...
export async function GET() {
  ...
  const rows = await sql`
    ...
    FROM dividend_calendar WHERE user_id = ${session.userId} AND account_id = ${accountId} ORDER BY date ASC
  `;
...
export async function POST(request: Request) {
  ...
  const [row] = await sql`
    INSERT INTO dividend_calendar (user_id, account_id, date, stock, type, note)
    VALUES (${session.userId}, ${accountId}, ${date}, ${stock}, ${type}, ${note ?? ""})
